    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Lecturer Enrollment</li>
        </ol>
      </nav>
    </div>
    <div class="row">
      ...
      ?>
        <section class="container">
          ...
          <form action="phpback/add_to_course_2.php" class="form" method="POST">
            <?php
              if (isset($_POST['lecturer_id'])) {
                  $lecturer_id = $_POST['lecturer_id'];

                  $stmt = $pdo->prepare("SELECT name FROM lecturers WHERE id = :lecturer_id");
                  $stmt->execute(['lecturer_id' => $lecturer_id]);
                  $lecturer = $stmt->fetch(PDO::FETCH_ASSOC);

                  if ($lecturer) {
                      $lecturer_name = $lecturer['name'];
                      echo '<input type="hidden" name="lecturer_id" value="' . $lecturer_id . '">';
                      echo "Lecturer name: " . $lecturer_name;
                  } else {
              ...
              <select name="institution_id" id="institution" required style="border: none; color: #707070; width: 100%; height: 50px; border: 1px solid #ddd; display: flex; border-radius: 6px;">
                ...
                <?php
                $stmt_inst_id = $pdo->prepare("SELECT institution_id FROM lecturers WHERE id = ?");
                $stmt_inst_id->execute([$lecturer_id]);
